...
这份文档把“Codex 接入任何绘画工具”的项目先收束成可执行路线。这里的绘画工具不只指传统画板，也包括图像生成、修图、设计稿、三维建模和工程制图工具。原则是：先稳定本机已经实践过的 ComfyUI、Blender、CAD 三条桥，再评估 Photoshop、Penpot/Figma、Krita 等扩展工具。
...
| `examples/bridge_status.py` | 一次检查 ComfyUI、Blender、CAD 的本机可用性 |
...
| Photoshop 修图 | `StarBoze/Photoshop-MCP-Server` | Windows 上可通过 PowerShell 或 UXP 插件控制 Photoshop，适合批处理、缩略图、自动修图 | 需要本机 Photoshop 和 Adobe/UXP 环境；订阅、登录和插件授权必须人工完成 |
...
6. Photoshop、Penpot/Figma 等扩展工具先做评估表和最小探针；凡是需要登录、订阅、浏览器授权、验证码或账号审批的步骤，都停下来让人手动处理。
